' governance-apply-active.vbs
' Apply governance to whatever file is CURRENTLY OPEN (ActiveProject)

Option Explicit
Dim msProj, activeProj, i, task, stamped, colored
...
' Use active project (whatever is open)
Set activeProj = msProj.ActiveProject
If activeProj Is Nothing Then
    WScript.Echo "ERROR: No project currently open"
    WScript.Quit 1
End If

WScript.Echo "Governance Views Setup"
WScript.Echo "====================="
WScript.Echo "[1/4] Found: " & Left(activeProj.Name, 60) & "..."
...
For i = 0 To 9
    Set task = activeProj.Tasks(gates(i, 0))
...
On Error Resume Next
For i = 10 To 12
    For Each task In activeProj.Tasks
        If Not task Is Nothing Then
...
On Error Resume Next
activeProj.TaskTables("Governance CPM Check").Delete
On Error GoTo 0

On Error Resume Next
Dim tbl, tf
Set tbl = activeProj.TaskTables.Add()
...
On Error Resume Next
activeProj.TaskFilters("Governance Gates Only").Delete
On Error GoTo 0

On Error Resume Next
Dim flt, fc
Set flt = activeProj.TaskFilters.Add()
...
activeProj.TaskTables("Governance CPM Check").Apply
activeProj.TaskFilters("Governance Gates Only").Apply
On Error GoTo 0
...
WScript.Echo "Filter: Governance Gates Only (visible rows)"
WScript.Echo ""
WScript.Echo "Colors:"
